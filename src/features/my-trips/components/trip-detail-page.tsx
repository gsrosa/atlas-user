...
import { applyPlanModification, type ItineraryDoc } from '../services/plan-edit'
...
const parseDays = (itinerary: Record<string, unknown>): Array<Record<string, unknown>> =>
  Array.isArray(itinerary.days) ? (itinerary.days as Array<Record<string, unknown>>) : []

const getUniqueCities = (days: Array<Record<string, unknown>>): string[] => [
  ...new Set(
    days
...
const computeTotalCost = (days: Array<Record<string, unknown>>): number =>
...
const parseAttractions = (day: Record<string, unknown>) => {
...
const buildGoogleMapsRouteUrl = (
  ...
  const waypoints = attractions
    .map((a) => a.address?.trim() || `${a.name}, ${city}`)
    .filter(Boolean)
  if (waypoints.length < 2) return undefined
  return `https://www.google.com/maps/dir/${waypoints.map(encodeURIComponent).join('/')}`
}

type MetaFlight = { flightNumber: string; departureDate?: string; arrivalDate?: string }
type MetaHotel = { name: string; checkinDate?: string; checkoutDate?: string; neighborhood?: string }

const isoToDayNumber = (iso: string, base: Date): number => {
  const d = new Date(iso)
  d.setHours(0, 0, 0, 0)
  return Math.round((d.getTime() - base.getTime()) / 86_400_000) + 1
}

const getHotelDayRange = (
...
const EditPanel = ({
  ...
  const buildAdaptPrompt = (values: EditValues): string | null => {
    const parts: string[] = []

    const localDatesChanged =
      values.departureAt !== initialValues.departureAt ||
      values.arrivalAt !== initialValues.arrivalAt

    if (localDatesChanged) {
      parts.push(
    ...
    const filledHotels = values.hotels.filter(
      (h) => h.name.trim() && h.checkinDate && h.checkoutDate,
    )
    ...
    if (filledHotels.length > 0) {
      const lines = filledHotels.map((h) => {
        ...
        if (!range)
          return `- ${h.name}: check-in ${h.checkinDate}, check-out ${h.checkoutDate}`
        const stayEnd = range.checkoutDay - 1
        const dayLabel =
          range.checkinDay === stayEnd
  ...
  const doSave = (values: EditValues, itinerary: ItineraryDoc) => {
    updatePlan.mutate({
      id: tripId,
      ...
      itinerary: {
        ...itinerary,
        meta: {
          ...((itinerary.meta as Record<string, unknown>) ?? {}),
          hotels: values.hotels.filter((h) => h.name.trim()),
        },
      },
    })
  }

  const handleSave = handleSubmit((values) => doSave(values, currentItinerary))

  const handleSaveAdapt = handleSubmit(async (values) => {
    const prompt = buildAdaptPrompt(values)
    if (!prompt) {
      doSave(values, currentItinerary)
      return
    }
  ...
  return (
    <form
      ...
      className="flex h-full flex-col gap-5 overflow-y-auto px-1 py-1"
    >
      {/* Title */}
      <div className="space-y-1">
        <label className={cls.label} htmlFor="edit-title">
          Trip name
        </label>
        <input
          id="edit-title"
      ...
      <div className="space-y-1">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className={cls.label} htmlFor="edit-departure">
              Departure
            </label>
            <input
              id="edit-departure"
          ...
          <div className="space-y-1">
            <label className={cls.label} htmlFor="edit-return">
              Return
            </label>
            <input
              id="edit-return"
        ...
        {datesChanged && (
          <div className="mt-2 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
            ...
            Dates changed — use{' '}
            <span className="font-semibold">"Save &amp; Adapt Plan"</span> to let AI
            replan the itinerary for the new dates.
          </div>
        )}
      </div>
      ...
      <div className={cls.section}>
        <div className={cls.sectionHeader}>
          ...
          <button
            type="button"
            onClick={() =>
              appendFlight({ flightNumber: '', departureDate: '', arrivalDate: '' })
            }
          ...
          <div key={field.id} className={cls.card}>
            <div className="flex gap-2">
              <input
                ...
                {...register(`flights.${idx}.flightNumber`)}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                ...
                <input
                  type="date"
                  {...register(`flights.${idx}.departureDate`)}
                  className={cls.input}
                />
              </div>
              <div className="space-y-1">
                ...
                <input
                  type="date"
                  {...register(`flights.${idx}.arrivalDate`)}
                  className={cls.input}
                />
              </div>
            </div>
      ...
      <div className={cls.section}>
        <div className={cls.sectionHeader}>
          ...
          <button
            type="button"
            onClick={() => appendHotel({ name: '', checkinDate: '', checkoutDate: '' })}
            className={cls.addBtn}
          >
            + Add
          </button>
          ...
          <div key={field.id} className={cls.card}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                ...
                <input
                  type="date"
                  {...register(`hotels.${idx}.checkinDate`)}
                  className={cls.input}
                />
              </div>
              <div className="space-y-1">
                ...
                <input
                  type="date"
                  {...register(`hotels.${idx}.checkoutDate`)}
                  className={cls.input}
                />
              </div>
            </div>
      ...
      <div className="flex flex-col gap-2 pt-1">
        <button
          type="button"
          onClick={() => void handleSaveAdapt()}
          disabled={isBusy}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary-500 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-50"
        >
...
const PlanView = ({
  ...
  const displayFlights: MetaFlight[] =
    metaFlights ?? flightNumbers.map((n) => ({ flightNumber: n }))
            ...
            <div className="flex flex-wrap gap-2">
              {displayFlights.map((f, i) => (
                <span
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full border border-surface-border bg-neutral-700 px-3 py-1.5 text-xs font-medium text-neutral-100"
                >
                  ...
                  {(h.checkinDate ?? h.checkoutDate) && (
                    <span className="text-neutral-400">
                      {h.checkinDate && h.checkoutDate
                        ...
                        : h.checkinDate
                          ? ` from ${h.checkinDate}`
                          : ` until ${h.checkoutDate}`}
                    </span>
                  )}
                  {h.neighborhood && (
                    <span className="text-neutral-400"> · {h.neighborhood}</span>
                  )}
                </span>
              ))}
            </div>
      ...
      {days.length > 0 && (
        <section className="space-y-0">
          {days.map((day, i) => {
            ...
            const city = typeof day.city === 'string' ? day.city : ''
            const dayAttractions = parseAttractions(day)
            return (
              <TripDayStepper
                key={dayNum}
                dayNumber={dayNum}
                ...
                mapsUrl={buildGoogleMapsRouteUrl(dayAttractions, city)}
              />
            )
          })}
        </section>
...
export const TripDetailPage = ({ tripId }: TripDetailPageProps) => {
    ...
    return (
      <AccountShell>
        <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-4 text-center">
          ...
          <button
            type="button"
            onClick={() => window.location.assign('/my-trips')}
  ...
  return (
    <AccountShell>
      <div
        className={`animate-account-fade-in-up mx-auto w-full px-4 py-8 sm:px-6 ${isEditOpen ? 'max-w-6xl' : 'max-w-2xl'}`}
      >
        {/* Header */}
        <div className="mb-8 flex items-start justify-between gap-4">
          <button
            type="button"
            onClick={() => window.location.assign('/my-trips')}
            className="mt-1 flex items-center gap-1 text-sm text-neutral-400 transition-colors hover:text-neutral-100"
          >
          ...
          {isEditOpen ? (
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl border border-surface-border px-3 py-1.5 text-sm font-medium text-neutral-400 transition-colors hover:border-primary-400 hover:text-primary-400"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl border border-surface-border px-3 py-1.5 text-sm font-medium text-neutral-100 transition-colors hover:border-primary-400 hover:text-primary-400"
            >
